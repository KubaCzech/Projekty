...
def calculate_marginal_concordance_matrix(
    ...
    boundary_profiles = boundary_profiles.reset_index(drop=True)
    ...
    for row1_id, row in dataset.iterrows():
        for row2_id, row2 in boundary_profiles.iterrows():
            for ix in range(len(row)):
                ...
                cr_type = criterion_types["type"].iloc[ix]
                q = indifference_thresholds[cr_name].iloc[row2_id]
                p = preference_thresholds[cr_name].iloc[row2_id]
    ...
    #         for k in range(concordance_matrix.shape[2]):
                # print(f"C(a{j+1}, b{k+1})")
                # print(concordance_matrix[i,j,k])
...
def calculate_comprehensive_concordance_matrix(
    ...
    """
    weights = criterion_types['k'].values
    ...
    for row1_id, row in dataset.iterrows():
        for row2_id, row2 in boundary_profiles.iterrows():
            for ix in range(len(row)):
                cr_name = dataset.columns[ix]
                cr_type = criterion_types["type"].iloc[ix]
                v = veto_thresholds[cr_name].iloc[row2_id]
                p = preference_thresholds[cr_name].iloc[row2_id]
...
def calculate_relation(
    ...
    """
    relation_matrix = np.full((len(alternatives), len(boundary_profiles_names)), '', dtype='<U1')
    # print("Relation matrix shape:",relation_matrix.shape)
...
def calculate_pessimistic_assigment(relation: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    print(relation)
    assignment = np.full((len(relation.index)), '', dtype='<U10')
    relation = relation.reset_index(drop=True)
    for idx, row in relation.iterrows():
        prev = '>'
        row = np.concat([row.values, np.array(['<'])])
        for i in range(len(row)):
            if (prev == '>' or prev == 'I') and (row[i] =='<' or row[i] == '?'):
                assignment[idx] = 'C' + str(i+1)
                break
            else:
                prev = row[i]
    return assignment
