...
def is_graph(graph: dict[int, list[int]]):
...
def check_and_copy_graph(graph: dict[int, list[int]]):
...
def count_odd_degrees(graph: dict[int, list[int]]):
    """O(v)"""
    return sum(len(terminal_vertices) % 2 == 1 for terminal_vertices in graph.values())


def has_euler_trail(connected_graph: dict[int, list[int]]):
    """O(v)"""
...
class DFS:
    ...
    def traverse(self, initial_vertex: int, graph: dict[int, list[int]]) -> deque:
        """O(v)"""
...
def get_connected_graph(graph: dict[int, list[int]], connected_graph_node_list: deque):
    """O(v)"""
    return {key: value for key, value in graph.items() if key in connected_graph_node_list}


def get_connected_graph_list(graph: dict[int, list[int]]) -> list[deque]:
    """O(v^2)"""
    dfs = DFS()
...
def find_hamiltonian_cycle_using_backtracking(connected_graph: dict[int, list[int]], path: list[int]) -> list[int]:
    """O(v!)"""
    connected_graph = check_and_copy_graph(connected_graph)
